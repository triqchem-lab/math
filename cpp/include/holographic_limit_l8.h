...
// ═══════════════════════════════════════════════════════
/* ── 精确性分层 (384k 步无漂移机制) ──
 * 精确状态 (零误差, 有限格点模运算): exp_3 / exp_2 / total_wraps —
 *   频率 f = 432 × 3^a × 2^b 的指数对表示, 演化是确定性模运算,
 *   不存在舍入误差可累积的通道 (C3/A4 轮转 = 群置换, 陈数 = 方向整数)。
 * 派生投影 (人类可读, Q16 有界误差): freq_log10_q16 / progress —
 *   波段判定边界宽 (数量级间隔), 投影误差 ≤10 lsb 不可能翻转分类。
 * 仲吕闭合相位对齐: acc=k·2¹⁶ 格点上 (acc×3¹¹)>>16 精确无损。
 */
